"""Latency stats across ALL FIVE real fillers (a/b/c/d/s), each forced via
RoutingContext.forced_strategy so every strategy runs regardless of what
Router's allocators would normally pick. One run per query x strategy
(c/d make real LLM/web calls -- expensive; not repeating 3x here, this is
a latency-shape snapshot, not the determinism check).

Usage (from mobius-rag/):
    .venv/bin/python scripts/run_latency_by_strategy_v2.py
"""
    ...
    async with AsyncSessionLocal() as db:
        for query in QUERIES:
            ...
            for strategy in STRATEGIES:
                ctx = RoutingContext(
                    query=query,
                    agent_id="retriever-latency-check-v2",
                ...
                try:
                    filled_shape = await _run_fillers_simple(
                        ...
                        query, gate_result, payer_context,
                    )
                    fillers_ms = int((time.monotonic() - t_fillers) * 1000)
                    occ = filled_shape.total_chunks_assigned
                    err = None
                ...
                rows.append({
                    ...
                    "react_ms": react_ms, "occupancy": occ, "error": err,
                })
                print(f"{query[:35]!r:38s} strategy={strategy:2s} "
                      f"router={router_ms:5d}ms fillers={fillers_ms:6d}ms react={react_ms:6d}ms "
                      f"occ={occ} {('ERR: ' + err) if err else ''}")

    print("\n" + "=" * 100)
    print("SUMMARY -- react_ms (router+fillers) by strategy, across all 3 queries")
    print("=" * 100)
    from collections import defaultdict
    import statistics
    by_strategy = defaultdict(list)
    for r in rows:
        by_strategy[r["strategy"]].append(r["react_ms"])
    header = f"{'strategy':10s} {'n':>3s} {'min':>8s} {'avg':>8s} {'max':>8s}"
    print(header)
    print("-" * len(header))
    for strategy in STRATEGIES:
        vals = by_strategy[strategy]
        print(f"{strategy:10s} {len(vals):>3d} {min(vals):>8d} {statistics.mean(vals):>8.0f} {max(vals):>8d}")
